...
mutual
  ...
  sub-Γ⊢τ {k = k} {ε = ε} Δ εδ <: (TWF-Base {ε₁ = ε₁} {ε₂ = ε₂} ε₁δ ε₂δ)
  ...
  sub-Γ⊢τ {k = k} {ε = ε} Δ εδ <: (TWF-Arr {τ₂ = τ₂} arrδ resδ)
  ...
  sub-Γ⊢τ Δ εδ <:δ (TWF-ADT consδs) = TWF-ADT (sub-cons Δ εδ <:δ consδs)
    where
  ...
  sub-Γ⊢ε⦂τ {σ = σ} {k = k} {φ = φ} {ε = ε} Δ εδ <: (T-Var {ι = ι} Γok τ-∈) with ctx-idx k <>? ι
  ... | less rep<var = T-Var (sub-Γok Δ εδ <: Γok) (var-earlier-in-Γ-remains Δ τ-∈ rep<var)
  ... | equal refl
        rewrite ∈-at-concat-point Δ τ-∈
              | replace-weakened-τ k (weaken-ε-k k ε) σ
              = {! !} -- Γ⊢ε⦂τ-weakening-suffix (sub-Γok Δ εδ <: Γok) {! !}
  ... | greater rep>var = T-Var (sub-Γok Δ εδ <: Γok) (var-later-in-Γ-remains Δ τ-∈ rep>var)
  sub-Γ⊢ε⦂τ {k = k} {Γ = Γ} {φ = φ} {ε = ε} Δ εδ <: (T-Abs {τ₁ = τ₁} {τ₂ = τ₂} {ε = ε'} arrδ bodyδ) = T-Abs (sub-Γ⊢τ Δ εδ <: arrδ) bodyδ'
    where
    bodyδ' : Γ ++ [↦Δ ε ] (Δ , τ₁) ⊢[ φ ]
...
{-
  sub-Γ⊢ε⦂τ {ℓ = ℓ} {k = k} {Γ = Γ} {φ = φ} {ε = ε}
    ...
    rewrite subst-commutes-τ-zero (ctx-idx k) (R.weaken-ε-k _ ε) ε₂ τ₂
          = T-App ε₁δ' (sub-Γ⊢ε⦂τ Δ εδ ε₂δ) (sub-Γ⊢τ<:τ' Δ εδ <:)
    where
    ε₁δ' : Γ ++ [↦Δ ε ] Δ ⊢[ φ ]
           [ ℓ ↦ε< ε ] ε₁ ⦂
