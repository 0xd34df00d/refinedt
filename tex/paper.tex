...
\begin{figure}[ht]
  ...
  \drules[T]{$\nonterm G \vdash \nonterm{es} : \nonterm{ts}$}{term typing}{Unit,Var,Abs,App,Case,Sub}
...
We introduce a shorthand notation $\top$ denoting a tautologically true \emph{proposition},
used for trivial refinements that do not carry any extra information.
One good way to define $\top$ is the equality of the sole member of $\Unit$ to itself:
...
We take $\lambda C$ \cite{Nederpelt14} enriched with case-splitting inspired by \cite{TAPLVariants,EisenbergThesis} as our core dependently typed language.
