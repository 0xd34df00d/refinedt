  ...
  \item \ranchor{TCTX-Bind}.
    \begin{align*}
      & \inferrule{\nonterm G \text{~ok} \\ \nonterm G \vdash \nonterm{ts}}
                  {\nonterm G , \gterm x : \nonterm{ts} \text{~ok}}
          \longmapsto
        \Tranctx(\nonterm G \text{~ok}), x : \tranty(\nonterm G \vdash \nonterm{ts}).
    \end{align*}

    We recurse on the prefix $\nonterm G$,
    which we are allowed to do due to the $\nonterm G \text{~ok}$ premise,
    and we use $\tranty$ to get the translated type of $\gterm x$,
    which we are also allowed to do due to the corresponding premise.
\end{itemize}
...
\begin{itemize}
  \item \ranchor{T-Unit}.
    ...
    We use $\tranctx$ to translate the surface derivation of that premise into the core one,
...
\begin{lemma}[Translation totality]
  $\tranctx$, $\Tranty$ and $\Tranterm$ are total.
\end{lemma}
\begin{proof}
